...
def train(epoch, model, optimizer, scheduler, loss_function, train_loader,
    ...
    start = time.time()
    for step, (images, poses, gazes, imEyeL, imEyeR) in enumerate(train_loader):
        ...
        if config.mode == GazeEstimationMethod.MPIIGaze.name:
            ...
            outputs = model(images, imEyeL, imEyeR, poses)
            # outputs = model(images,  imEyeR, poses)
        else:
            raise ValueError
...
def validate(epoch, model, loss_function, val_loader, config,
    ...
    with torch.no_grad():
        for step, (images, poses, gazes, imEyeL, imEyeR) in enumerate(val_loader):
