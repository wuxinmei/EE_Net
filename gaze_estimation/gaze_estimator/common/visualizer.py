...
class Visualizer:
    ...
    def draw_3d_gaze(self,
                     point0: np.ndarray,
                     point1: np.ndarray,
                     color: Tuple[int, int, int] = (64, 0, 215),
                     lw=2) -> None:
        assert self.image is not None
        assert point0.shape == point1.shape == (3, )
        points3d = np.vstack([point0, point1])
    ...
    def show_gaze(self,
                     ...
                     pitch: np.ndarray,
                     yaw: np.ndarray,
                     color: Tuple[int, int, int] = (64, 0, 215)) -> None:
        assert self.image is not None
        cv2.putText(self.image, f' ground_truth pitch: {pitch_:.2f}, yaw: {yaw_:.2f}', (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        cv2.putText(self.image, f' estimated pitch: {pitch:.2f}, yaw: {yaw:.2f}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
