...
class GazeEstimator:
    ...
    def __init__(self, config: yacs.config.CfgNode):
        ...
        self._normalized_camera_eye = Camera(
            config.gaze_estimator.normalized_camera_params_eye)
        ...
        self._head_pose_normalizer_eye = HeadPoseNormalizer(
            self.camera, self._normalized_camera_eye,
            self._config.gaze_estimator.normalized_camera_distance)
    ...
    def estimate_gaze(self, image: np.ndarray, face: Face) -> None:
        ...
        if self._config.mode == GazeEstimationMethod.MPIIGaze.name:
            ...
            for key in self.EYE_KEYS:
                eye = getattr(face, key.name.lower())
                self._head_pose_normalizer_eye.normalize(image, eye)
    ...
    def _run_mpiigaze_model(self, face: Face) -> None:
        ...
        for key in self.EYE_KEYS:
            ...
            normalized_head_pose = face.normalized_head_rot2d
    ...
    def _run_mpiifacegaze_model(self, face: Face) -> None:
        ...
        for key in self.EYE_KEYS:
            eye = getattr(face, key.name.lower())
            eyeimage = eye.normalized_image
            # print("save eye image......")
            # cv2.imwrite('experiments/eye.jpg', eyeimage)
            if key == FacePartsName.REYE:
                imEyeR = self._transform(eyeimage).unsqueeze(0)
            if key == FacePartsName.LEYE:
                imEyeL = self._transform(eyeimage).unsqueeze(0)
        head_poses = np.array(normalized_head_pose).astype(np.float32)
        head_poses = torch.from_numpy(head_poses).unsqueeze(0)
        imFace = face.normalized_image
        imFace = self._transform(imFace).unsqueeze(0)
        device = torch.device(self._config.device)
        ...
        with torch.no_grad():
            ...
            head_poses = head_poses.to(device)
            prediction = self._gaze_estimation_model(imFace, imEyeL, imEyeR, head_poses)
            prediction = prediction.cpu().numpy()
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open('prediction.csv', 'a', newline='') as csvfile:
            csv_writer = csv.writer(csvfile)
            for pred in prediction:
                csv_writer.writerow(([pred], current_time))
